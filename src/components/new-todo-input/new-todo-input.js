...
export default function NewTodoInput () {
  ...
  useEffect (()=> { // Если в TextArea нет текста он становится нормального размера 
    if (!inputValue.trim()) {
      setTextareaHeight (50)
    }
  },[inputValue])

  const textareaEnterSubmit = (e) => { // Enter для добавление ToDo, Shift + Enter для то что-бы перейти на новую строку
